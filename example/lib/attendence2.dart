...
class _AttendencePageState extends State<AttendencePage2> {
  ...
  Future<Null> _onTimeScore(BuildContext context) async {
    ...
    if (stime > stimes) {
      score = limitScoreController.text;
      print(score);
    } else if (stime < stimes) {
      score = realScoreController.text;
      print(score);
    } else {
      score = realScoreController.text;
      print(score);
      ...
      Fzxing.scan(isBeep: true, isContinuous: false).then((barcodeResult) {
        ...
        setState(() {
          ...
          for (int i = 0; i < _barcode.length; i++) {
            ...
            print("object : " + widget.qColumn.toString());
            postScore(barcode);
          }
          //snackbar("บันทึกแล้ว");
        });
